...
| **API keys** | Keys saved in Settings live in the browser's `localStorage` and travel per request; they are never persisted server-side. That is the price of a demo with no accounts: any script that runs on the page can read them. The mitigation is the Content-Security-Policy in [`next.config.ts`](../next.config.ts): no third-party script is loaded, and `connect-src 'self'` stops a script from sending them anywhere else. | Move key management to a secrets and identity layer rather than trusting the client: keys held server-side per user, never in the browser. |
...
`EVAL_MAX_CONCURRENT` (8) may run at once. The judge key is forwarded only over https or to the same machine, and the service
