...
| `npm run test:e2e:rubrics` | The Scoring Rubric button on every analysis page and its scrollable modal: the judge's rubric as short labelled steps (Trace shows all four), Meridian's own rules for each page (risk dimensions, compliance checks, citation identifying, guardrails), keyboard use, the accessibility scan, and the message when the evaluation service is off |
